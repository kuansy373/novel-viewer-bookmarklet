# Web小説一行化ブックマークレット
とりあえず使ってみたい方は [実行](#実行)  から、`javascript:`で始まるコードをコピーしてブックマークレットを作成、実行しましょう。
### 目次
- [できること](#できること)
- [作ったきっかけ](#作ったきっかけ)
...
- ※ 自動スクロールでカクつきが発生することがあります。（Chromeを使う、リフレッシュレートを60Hzにする、常時自動スクロールスライダーバーに触れる、などで改善することがあります）
...
- 上記以外のサイトでも使いたい場合や、作者あとがきを含めたくない場合など、自由に改変してください。
  ...
  - 小説本文のページで右クリックし、「開発者ツール」→「ページのソースを確認」から、縦一行にしたい文章を囲っているタグを確認することができます。
...
※ テキスト選択時の検索ショートカットの表示を無くすには、`"searchConfigs": []` と、中身を空にすることで可能です。
  - また、SAVEボタンについては、データをどこかストレージに保存しているのではなく、変数に保持しているだけなので、小説タブを閉じると同時に失われます。
  - JSONをそのままメモ帳などにペーストしておくことを想定しているので、タブを離れるときは保持しているJSONを忘れずにクリップボードへコピーしましょう。次回以降、入力欄に貼り付けて反映できます。
...
- v1.3.x: ページ切り替えとJSON保存時のconfirmをモーダルUI化。JSONの貼り付け反映を実装。HTMLエスケープ追加。
